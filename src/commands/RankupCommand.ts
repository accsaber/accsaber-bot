...
export default class RankupCommand implements Command {
  ...
  public slashCommandBuilder = new SlashCommandBuilder()
    .setName("rankup")
    .setDescription(
      "Get the role for the highest level you've reached in the campaign",
    );
  ...
  public async execute(interaction: CommandInteraction) {
    ...
    if (!interaction.member) {
      await interaction.reply(
        "This command can only be executed from within the server.",
      );
      return;
    }

    // Get SS ID
    const accSaberUser = await AccSaberUser.findOne({
    ...
    try {
      const response = await axios.get(
        `https://campaigns.accsaber.com/api/0/player-campaign-infos/${accSaberUser.scoreSaberID}`,
      );
    ...
    for (const milestone of milestones) {
      if (milestone.milestoneId > currentMilestone) {
        ...
        await memberRoleManager.add(
          RankupCommand.rankRoleIDs[milestone.milestoneId],
        );
        if (currentMilestone !== -1) {
          // Don't remove the champ role on rankup (1 = champ)
          if (currentMilestone !== 1)
            await memberRoleManager.remove(
              RankupCommand.rankRoleIDs[currentMilestone],
            );
        } else {
